...
ALL_FLAGS = $(basename $(notdir $(wildcard $(FLAGS_SRC_DIR)/emoji_u*.png)))
...
EMOJI_NAMES = $(notdir $(wildcard $(EMOJI_SRC_DIR)/*.png))
...
%.ttx: %.ttx.tmpl $(ADD_GLYPHS) $(ALL_COMPRESSED_FILES)
	...
	@python $(ADD_GLYPHS) -f "$<" -o "$@" -d "$(COMPRESSED_DIR)"
